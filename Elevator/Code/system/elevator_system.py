...
class ElevatorSystem:
    def __init__(self, num_elevators=2, max_floor=3, identity=None):
        ...
        self.caller_timer.timeout.connect(self.call_elevator)
        self.caller_timer.start(500)  # 每500毫秒运行一次
        # caller.start()
        for elevator in self.elevators:
            elevator.caller_timer = QTimer()
            elevator.caller_timer.timeout.connect(elevator.move)
            elevator.caller_timer.start(100 + elevator.id * 25)  # 
