...
class TestFrontendUI(unittest.TestCase):
    ...
    def test_on_api(self):
        ...
        # 读取 API 文件
        filepath = r"./Development/HuarongdaoFinalTestCase/testcase.txt"
        ...
        for api in lines:
            ...
            if api.startswith("set@"):
                val = api.split("@", 1)[1]
                if val == "03#203#2116511857944":
                    # 点击 Level 样例
                    btn = getattr(self.win, "level_btn")
                    QTest.mouseClick(btn, Qt.LeftButton)
                    QTest.qWait(200)
                    menu = self.win.level_menu
                    ...
                    for act in menu.actions():
                        if act.text() == "最终测试":
                            ...
                            QTest.qWait(200)
                            break
            elif api.startswith("move@"):
                payload = api.split("@", 1)[1]
                name, direction = payload.split("#")
                idx = API_DICT.get(name)
                # 点击棋子
                # print(self.win.board_widget.rect_map)
                rect = self.win.board_widget.rect_map[idx]
                pt = self.win.board_widget.mapToGlobal(rect.center())
                pyautogui.click(pt.x(), pt.y())
                QTest.qWait(100)
                # 点击方向按钮
                btn = getattr(self.win, f"{direction.lower()}_btn", None)
                self.assertIsNotNone(btn, f"{direction}_btn 未找到")
                QTest.mouseClick(btn, Qt.LeftButton)
                QTest.qWait(100)
            elif api == "undo":
                btn = getattr(self.win, "undo_btn", None)
                self.assertIsNotNone(btn, "undo_btn 未找到")
                QTest.mouseClick(btn, Qt.LeftButton)
                QTest.qWait(100)
